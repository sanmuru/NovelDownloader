...
{
    [NovelDownLoadPluginBookToken(QiDian_NovelDownloader._guidStr)]
    public class BookToken : NDTBook
    {
        internal static readonly Regex BookUrlRegex = new Regex(@"^http(s)?://book.qidian.com/info/(?<BookUnicode>\d*)$", RegexOptions.Compiled);
        internal static readonly Regex CategoryUrlRegex = new Regex(@"^http(s)?://book.qidian.com/info/(?<BookUnicode>\d*)#Catalog$", RegexOptions.Compiled);
        ...
        public BookToken(Uri uri) : base(uri)
        {
            ...
            HtmlDocument doc = web.Load(this.BookUrl);
