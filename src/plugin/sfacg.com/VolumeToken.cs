...
namespace SamLu.NovelDownloader.Plugin.sfacg.com
{
	public class VolumeToken : NDTVolume
	{
		...
		private string[] Creep()
		{
			...
			return new string[]
			{
				current.GetAttributeValue("title", string.Empty).Trim(),
				new Uri(NovelDownloader.HostUri, current.GetAttributeValue("href", null)).ToString()
			};
		}
		...
		protected override bool CreepInternal()
		{
			...
			if (data != null && data.Length == 2)
			{
                ...
                if (ChapterToken.FreeChapterUrlRegex.IsMatch(data[1]))
                    chapterToken = new FreeChapterToken(uri);
                else
                    chapterToken = new VipChapterToken(uri);
                chapterToken.Title = data[0];
                chapterToken.BookUnicode = this.BookUnicode;
                chapterToken.VolumeUnicode = this.VolumeUnicode;
